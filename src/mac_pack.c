...
float mac_pack_probability(const ble_device_t *older, const ble_device_t *newer) {
    ...
    if (gap < 0) gap = 0;
    return expf((float)-gap / MAC_PACK_TIME_CONSTANT_SEC);
}
...
void mac_pack_run(device_list_t *list) {
    ...
    for (int i = 0; i < active_count; i++) {
        ...
        for (int j = 0; j < active_count; j++) {
            ...
            if (mac_pack_overlaps(older->first_seen, older->last_seen,
                                   newer->first_seen, newer->last_seen)) continue;
            ...
            float p = mac_pack_probability(older, newer);
            if (p > best_prob) {
                best = older;
                best_prob = p;
            }
